...
def data_prep(datasets_path, admin_file, diag_file, procedure_file=None, 
              prescript_file=None, drug_file=None):
    ...
    if procedure_file and prescript_file and drug_file:
        ...
        df_adm['num_days'] = (df_adm['dischtime'] - df_adm['admittime']).dt.days + 1
        df_adm = df_adm.rename(columns={'hospital_expire_flag':'death_flag'})
        df_diags['icd9_code'] = df_diags['icd9_code'].astype(str)
        ...
        df2 = df.groupby(['subject_id', 'hadm_id']).agg(set)

        df2['icd9_diag'] = df2['icd9_diag'].apply(list)
        df2['icd9_diag'] = df2['icd9_diag'].apply(sort_list)
        df2['icd9_diag'] = df2['icd9_diag'].apply(lambda x: list(dict.fromkeys(x)))
        df2['admittime'] = df2['admittime'].apply(lambda x: list(x)[0])
        df2['num_days'] = df2['num_days'].apply(lambda x: list(x)[0]).astype(str)

        df3 = df2.reset_index()
        df3['death_flag'] = df3['death_flag'].apply(lambda x: list(x)[0])

        data = []
        patient_ids = df3['subject_id'].unique()
        for p_id in patient_ids:
            patient = {}
            df_p_id = df3[df3['subject_id']==p_id].sort_values('admittime')
            adm_ids = df_p_id.loc[:,'hadm_id']
            visits = []
            for adm_id in adm_ids:
                df_visit = df3[(df3['subject_id']==p_id) & (df3['hadm_id']==adm_id)]
                visit = {}
                visit['admittime'] = df_visit['admittime'].values[0]
                visit['num_days'] = df_visit['num_days'].values[0]
                visit['death_flag'] = df_visit['death_flag'].values[0]
