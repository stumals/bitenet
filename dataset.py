...
def data_prep(datasets_path, admin_file, diag_file, procedures_file=None,
              min_visits=2, max_visits=10, code_count_threshold=5,
              dxtx=True, onehot=True):
    icd9_code_categories = list(range(19))

    df_adm = pd.read_csv(os.path.join(os.getcwd(), datasets_path, admin_file), dtype=str)
    df_adm.columns = df_adm.columns.str.lower()
    df_diags = pd.read_csv(os.path.join(os.getcwd(), datasets_path, diag_file), dtype=str)
    df_diags = df_diags.dropna()
    df_diags.columns = df_diags.columns.str.lower()
    if dxtx:
        df_proced = pd.read_csv(os.path.join(os.getcwd(), datasets_path, procedures_file), dtype=str)
    ...
    if dxtx:
        ...
        df2['icd9_code'] = df2[['icd9_diag', 'icd9_proced']].apply(set, axis=1).apply(
